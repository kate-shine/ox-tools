...
/// `.github/skills/code-review/SKILL.md` — review guidance for PR reviewers.
#[must_use]
pub fn code_review_skill() -> Artifact {
    Artifact::backend_file(Backend::GitHub, ".github/skills/code-review/SKILL.md", CODE_REVIEW_SKILL)
}
...
mod tests {
    ...
    /// The skill is loaded by name, so the front matter must survive edits, and
    /// the rule the pipeline exists to make redundant must stay stated.
    #[test]
    fn code_review_skill_declares_its_name_and_core_rules() {
        assert!(CODE_REVIEW_SKILL.starts_with("---\nname: code-review\n"));
        assert!(CODE_REVIEW_SKILL.contains("GENERATED BY cargo-anvil. DO NOT EDIT DIRECTLY."));
        assert!(CODE_REVIEW_SKILL.contains("Never predict build, lint, or test outcomes"));
        assert!(CODE_REVIEW_SKILL.contains("Do not assert an API from memory"));
        assert!(CODE_REVIEW_SKILL.contains("Convention claims need evidence from this repo"));
    }
